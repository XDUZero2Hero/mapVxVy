...
void Chassis::ChassisSpeedCtrlCallback(const geometry_msgs::Twist::ConstPtr &vel){
  ...
  chassis_speed.car_v = sqrt(chassis_speed.vx^2 + chassis_speed.vy^2);
  chassis_speed.map_vx = chassis_speed.car_v*sin(chassis_info->gyro_angle*10);
  chassis_speed.map_vy = chassis_speed.car_v*cos(chassis_info->gyro_angle*10);
...
/*
There weren't car_v map_vx map_yx before,.Why angle multiply 10,I got this from
function ChassisInfoCallback.
*/
